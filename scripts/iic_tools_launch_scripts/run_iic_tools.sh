...
if [ "$(docker ps -aq -f name="${CONTAINER_NAME}")" ]; then
	...
	#                    Run Config
	# ================================================
	SELECT_PDK="false" # set flag to TRUE choose PDK on each launch
	USER_PDK="sky130A" # Default PDK
	# ================================================
	#          Run with default/selected PDK
	# ================================================
	echo 
	echo "------------------------------------------------"
	echo "Current default PDK: ${USER_PDK}"
	# Fetch available PDKs, skip UI, and filter out all [INFO] lines

	# PDK selection
	if [ "${SELECT_PDK^^}" = "TRUE" ]; then
		# Fetch available PDKs, skip UI, and filter out all [INFO] lines and .txt files
		AVAILABLE_PDKS=$(docker run --rm "${DOCKER_USER}/${DOCKER_IMAGE}:${DOCKER_TAG}" --skip ls /foss/pdks/ 2>/dev/null | grep -v -e '^\[INFO\]' -e '\.txt$')
		echo "Available PDKs: "
		echo "${AVAILABLE_PDKS}"

		# Prompt the user for PDK
		echo -n "Enter the technology PDK you want to use (no input -> default PDK): "
		read INPUT_PDK

		if [ -z "$INPUT_PDK" ]; then
			echo "No input detected. Using default: ${USER_PDK}"
		else
			if echo "${AVAILABLE_PDKS}" | grep -xF "$INPUT_PDK"; then
				USER_PDK="$INPUT_PDK"
			else
				echo ""
				echo "[ERROR]: '${INPUT_PDK}' is not a valid PDK!"
				echo "Exiting script. Docker will not launch."
				echo "------------------------------------------------"
				exit 1
			fi
		fi
	fi
	echo "Launching Docker with PDK: ${USER_PDK}"
	echo "------------------------------------------------"
	echo 
	# Clean up docker container (-rm) and run docker with selected PDK
	${ECHO_IF_DRY_RUN} docker run -d --rm --user "${CONTAINER_USER}:${CONTAINER_GROUP}" -e "DISPLAY=${DISP}" -e "PDK=${USER_PDK}" -v "${DESIGNS}":"/foss/designs":rw ${PARAMS} --name "${CONTAINER_NAME}" "${DOCKER_USER}/${DOCKER_IMAGE}:${DOCKER_TAG}"
	# ================================================
fi
